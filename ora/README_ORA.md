...
The plotting commands accept ORA results generated by gProfiler, clusterProfiler, or fgsea as CSV, TSV, or RDS files. The examples below use gProfiler results from `enrichviz ora gprofiler`. After install, run them inside `pixi shell` (or as `pixi run enrichviz -- …`).
