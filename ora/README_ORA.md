...
The plotting scripts accept ORA results generated by gProfiler, clusterProfiler, or fgsea as as CSV, TSV, or RDS files. The examples below use gProfiler results generated with `gprof_GO.R`. From the repository root, `enrichviz ora gprofiler` and `enrichviz ora barplot` are equivalent to these `Rscript` commands (see the main README).

```bash
Rscript gprof_GO.R --in edger.csv --outdir ora_out --direction yes
```
...
```bash
Rscript ora_barplot.R --in ora_out/gprofiler_GO.csv --outdir ora_out --n 10 --ont BP
Rscript ora_barplot.R --in ora_out/kegg.csv --outdir ora_out --n 10 --prefix kegg
```
...
```bash
Rscript ora_dotplot.R --in ora_out/gprofiler_GO.csv --outdir ora_out --n 10 --ont BP
Rscript ora_dotplot.R --in ora_out/kegg.csv --outdir ora_out --n 10 --prefix kegg
```
...
## 3. Lollipop plot (`ora_lollipop.R`)
...
```bash
Rscript ora_lollipop.R --in ora_out/gprofiler_GO.csv --outdir ora_out --n 10 --ont BP
Rscript ora_lollipop.R --in ora_out/kegg.csv --outdir ora_out --n 10 --prefix kegg
```
...
```bash
Rscript ora_upsetplot.R --in ora_out/gprofiler_GO.csv --outdir ora_out --n 10 --ont BP --direction up
Rscript ora_upsetplot.R --in ora_out/kegg.csv --outdir ora_out --n 10 --direction none --prefix kegg
```
...
## 5. Semantic space plot (`ora_ssplot.R`)
...
```bash
Rscript ora_ssplot.R --in ora_out/gprofiler_GO.csv --outdir ora_out --n 30 --ont BP --direction up
Rscript ora_ssplot.R --in ora_out/kegg.csv --outdir ora_out --n 30 --direction none --prefix kegg
```
...
```bash
Rscript simplify_GO.R --in ora_out/gprofiler_GO.csv --outdir ora_out --ont BP --direction up --organism hsapiens
```
