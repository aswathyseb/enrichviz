# Usage:
#   Rscript gsea_barplot.R --in gsea_out/fgsea_GO_all.csv --outdir gsea_out -n 10 --ont BP [--prefix NAME]
#   Rscript gsea_barplot.R --in t5/fgsea_KEGG.csv --outdir gsea_out -n 10 [--prefix kegg]
#
# Barplot for GO or KEGG GSEA results from fgsea or clusterProfiler.
...
opt_parser <- OptionParser(
  usage = "\nRscript gsea_barplot.R --in FILE --outdir DIR -n 10 [--ont BP] [--prefix NAME]\n",
  option_list = option_list,
  description = paste(
    "Barplot of top GSEA terms from GO or KEGG.",
    ...
    "  Rscript gsea_barplot.R --in t5/fgsea_KEGG.csv --outdir gsea_out -n 10 --prefix kegg",
    "  Rscript gsea_barplot.R --in t5/gseaKEGG_clusterprof.csv --outdir gsea_out -n 10 --prefix kegg",
    sep = "\n"
  )
)
...
gsea_barplot_plot <- function(plot_df, title) {
  ggplot(plot_df, aes(x = NES, y = label, fill = p.adjust)) +
...
plot_csv <- file.path(RESULTS_DIR, gsea_out_name(paste0(plot_meta$tag, "_barplot_table.csv"), prefix))
write_csv(plot_df %>% mutate(label = as.character(label)), plot_csv)

n_rows <- nrow(plot_df)
plot_height <- max(5, 0.27 * n_rows + 2)

bar_file <- file.path(RESULTS_DIR, gsea_out_name(paste0(plot_meta$tag, "_barplot.pdf"), prefix))

ggsave(
  bar_file,
  plot = gsea_barplot_plot(plot_df, plot_meta$title),
  width = 9,
  height = plot_height
)
